...
| **Versão actual** | `0.8.53` (`versionCode` **54**) |
...
3. Aguardar processamento (1–5 min). Verificar:
   ...
   - Version name: `0.8.53` (ou a versão actual do `pubspec.yaml`)
   - Version code: `53` (número após `+` no pubspec)
4. **Nome da versão:** `0.8.53 (54)` (notas internas)
5. **Notas da versão** (visíveis aos testadores) — ver também `./scripts/play_console_brief.sh`:
   ```
   v0.8.53 — Correcções Finanças e táctica (build 54).
...
7. Problemas → pakopt7@gmail.com com versão (ex. `0.8.53 (54)`) e modelo do telemóvel
...
1. Editar `apps/phoenix_manager/pubspec.yaml`:
   ```yaml
   version: 0.8.53+54   # nome visível + versionCode (número após +)
   ```
2. Rebuild e upload:
   ```bash
   ./scripts/build_mobile.sh android
