...
| **Versão actual** | `0.8.26` (`versionCode` **27**) |
...
**Phoenix Manager (v0.8.26+):** escolhe **Gratuita** — sem anúncios, sem compras in-app, sem billing. A «receita» no jogo é simulação (finanças do clube), não monetização real.
...
3. Aguardar processamento (1–5 min). Verificar:
   ...
   - Version name: `0.8.26` (ou a versão actual do `pubspec.yaml`)
   - Version code: `24` (número após `+` no pubspec)
4. **Nome da versão:** `0.8.26 (27)` (notas internas)
...
7. Problemas → pakopt7@gmail.com com versão (ex. `0.8.26 (27)`) e modelo do telemóvel
...
1. Editar `apps/phoenix_manager/pubspec.yaml`:
   ```yaml
   version: 0.8.26+27   # nome visível + versionCode (número após +)
   ```
2. Rebuild e upload:
   ```bash
   ./scripts/build_mobile.sh android
