...
| **Versão actual** | `0.8.13` (`versionCode` **14**) |
...
**Project Phoenix Manager (v0.8.13+):** escolhe **Gratuita** — sem anúncios, sem compras in-app, sem billing. A «receita» no jogo é simulação (finanças do clube), não monetização real.
...
3. Aguardar processamento (1–5 min). Verificar:
   ...
   - Version name: `0.8.13` (ou a versão actual do `pubspec.yaml`)
   - Version code: `14` (número após `+` no pubspec)
4. **Nome da versão:** `0.8.13 (14)` (notas internas)
5. **Notas da versão** (visíveis aos testadores) — ver também `./scripts/play_console_brief.sh`:
   ```
   v0.8.13 — gestão offline com PSE; acessibilidade e polish UX.
   - Modo Express e Diretor · liga, taça, mercado, finanças
   - Acessibilidade: plantel, treino, placares, classificação
   - Desktop: tooltips no menu lateral · «Sair do jogo»
   - Edge-to-edge Android 15 · datas legíveis
...
7. Problemas → pakopt7@gmail.com com versão (ex. `0.8.13 (14)`) e modelo do telemóvel
...
1. Editar `apps/phoenix_manager/pubspec.yaml`:
   ```yaml
   version: 0.8.13+14   # nome visível + versionCode (número após +)
   ```
2. Rebuild e upload:
   ```bash
   ./scripts/build_mobile.sh android
