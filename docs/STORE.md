...
| **Versão actual** | `0.8.45` (`versionCode` **46**) |
...
**Phoenix Manager (v0.8.45+):** escolhe **Gratuita** — sem anúncios, sem compras in-app, sem billing. A «receita» no jogo é simulação (finanças do clube), não monetização real.
...
3. Aguardar processamento (1–5 min). Verificar:
   ...
   - Version name: `0.8.45` (ou a versão actual do `pubspec.yaml`)
   - Version code: `46` (número após `+` no pubspec)
4. **Nome da versão:** `0.8.45 (46)` (notas internas)
5. **Notas da versão** (visíveis aos testadores) — ver também `./scripts/play_console_brief.sh`:
   ```
   v0.8.45 — Emblemas na Dashboard (build 46).
   - Crests junto aos nomes dos clubes
   - Próximo jogo, lista e mini-tabela
...
7. Problemas → pakopt7@gmail.com com versão (ex. `0.8.45 (46)`) e modelo do telemóvel
...
1. Editar `apps/phoenix_manager/pubspec.yaml`:
   ```yaml
   version: 0.8.45+46   # nome visível + versionCode (número após +)
   ```
2. Rebuild e upload:
   ```bash
   ./scripts/build_mobile.sh android
